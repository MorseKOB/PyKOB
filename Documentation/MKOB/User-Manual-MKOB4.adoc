...
Like all computer programs MKOB (((MKOB, MKOB4))) has lots of configuration settings and tricks for making the best use of it.
This user's manual attempts to cover all of them and gives a good idea of how to set it up.

Don't let the apparent complexity intimidate you. At its heart MKOB is simply a tool for learning and using landline telegraphy on a computer. MKOB (((MKOB, MKOB4))) runs on Windows, Linux, and Mac, and people are porting it to other systems like phones and embedded devices. It can be used with a real telegraph key and sounder or it can realistically simulate them.
...
===== Keyboard Shortcuts
Opens a separate window that displays the keyboard shortcuts available in MKOB4.
...
===== About
Lists version information for MKOB4, PyKOB, and some of the primary modules used.
