...
== Related Components
=== pykob Package
((MKOB)) uses the *pykob*((pykob)) package(((package, pykob))) of modules. The ((pykob)) package consists of a number of modules that provide different aspects of handling code, the physical key and sounder, internet connection to a KOBServer instance, system audio to simulate a sounder or provide tone, configuration and logging capabilities, and more. When updating ((MKOB)), it is advisable to get the latest (associated) release of ((pykob)) as well.
