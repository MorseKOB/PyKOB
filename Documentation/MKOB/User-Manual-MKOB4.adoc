...
== Related Components
=== pykob Package
((MKOB)) uses the *((pykob))* package(((package, pykob))) of modules. The ((pykob)) package consists of a number of modules that provide different aspects of handling code, the physical key and sounder, internet connection to a KOBServer instance, system audio to simulate a sounder or provide tone, configuration and logging capabilities, and more. When updating ((MKOB)), it is advisable to get the latest (associated) release of ((pykob)) as well.
...
=== Telegram
((Telegram)) is an application specifically designed for use in displays. ((Telegram)) presents a full screen
interface with no exposed controls. Its interface appears as a telegram form that fills
...
[%nonfacing]
=== Comparison to MorseKOB 2.5(((MorseKOB, MorseKOB 2.5)))
This is the one place where the operation of ((MKOB)) will be compared to that of MorseKOB 2.5(((MorseKOB, MorseKOB 2.5))). If you do not have experience with MorseKOB 2.5(((MorseKOB, MorseKOB 2.5))) you can skip the rest of this section.

MorseKOB 2.5(((MorseKOB, MorseKOB 2.5))) doesn’t track the state of the physical key’s closer in its interface. It treats its keyboard sender, its system (computer) audio sounder, and its Circuit Closer checkbox as a ‘separate circuit’ that is connected in series with the connected physical key and sounder (loop) and the KOBServer wire (circuit) that others are also connected into.
...
However, when using a ((KOB/Loop)) you need to have the physical key closed in order for the sounder to sound. If you want to send from the ((Keyboard Sender)) and hear the code on the sounder, you can leave the physical key closed and uncheck the ((Key Closed)) box and then send from the keyboard. Opening and closing the physical key will once again sync up the Key Closed state in the interface to the state of the key. Similarly, with the key's closer open, you can check the Key Closed box and ((MKOB)) will treat the circuit as closed. Again, closing the key will re-sync the interface to the state of the key.
...
[%nonfacing]
=== Code Reader (Copy)
Copied (decoded) code is displayed in this window. Code is copied from the local key and from stations on the wire when connected. The Reader window is also occasionally used to display information from the ((MKOB)) application to the user. (((Code Reader)))

==== Code Format and Information Display
Some information about the received code and some formatting is performed by the Reader as the code is displayed.

When you connect to a wire and code is first received, the Office/Station ID of the sender is printed within angle brackets - _<Office ID>_. The Office/Station ID is also printed when the sender changes. When a paragraph mark ('=') is received the Reader will go to a new line on the next non paragraph mark received. When the sender closes their key an underscore is printed. When you disconnect from a wire (or get disconnected) _pass:['#####']_ is printed. If the reader detects that the spacing of a character isn't as expected it displays the character with a small under-tick(((under-tick, Code Reader))).
...
==== Order and Format of Listed Offices
...
Operations pertaining to application preferences (configuration)(((configuration, aka preferences))).
...
===== Active Wires
Opens a separate window that displays the currently active wires on the KOBServer. The list can be used to select a wire to connect to (single click) and to connect (double click). If already connected, it will switch the wire.
...
[%nonfacing]
== Preferences
All of the application preferences (also referred to as the configuration) can be set using the Preferences Dialog accessed using the File:Preferences… menu. They can also be listed and set using the *((Configure))* (((Configure, Command line utility))) command line utility. Some of the configuration can also be set using the operational controls in the interface.
...
** None (((None, interface)))– No interface is to be used
** Serial Port (((Serial, interface)))– A serial interface to either a key, a sounder, or a KOB, is used. The drop-down menu contains a list of serial devices found. An interface value that isn't currently available (or doesn't show up) can be entered manually.
+
The special value _SDIF_ (((SDIF, SilkyDESIGN-Interface))) can be manually entered to have MKOB look for a SilkyDESIGN-Interface when it runs.
...
*** Tone – Use a radio-style (CW) ((tone)) (((tone, system sound)))
* Use local sounder:
...
**New in MKOB ((version 4.4.1))**, the ((Key and sounder interface)) can be set to _None_ (((None, interface))) while selecting/retaining a value for the port (the value in the entry field). This allows disabling the interface while keeping the port value for later use.
...
+
The speeds and spacing option (((Farnsworth, speed and spacing))) can also be set in the main application interface.
...
* *((Decode Morse at detected speed))*:
** When checked, MKOB will use the speed that it is detecting for the incoming code to decode and display the text.
** When unchecked, MKOB will use the configured speed (the *Character speed*) to decode and display the text.
...
Since a closed-loop telegraph has the sounder energized whenever it is idle, this feature allows power to be saved by de-energizing the sounder after a period of no activity. When the specified number of seconds elapses with no activity of the sounder, the sounder will be powered off. Any activity, whether received from a connected wire or initiated on the ((local key)) or ((Keyboard Sender)), will re-energize the sounder (((sounder, re-energize))) and normal operation will continue. This can be very helpful when the KOB/loop is being powered by a battery.
+
A value of zero (0) disables this functionality.(((Sounder power save, disable)))
...
** *Extend key open check*:
This extends the length of time that the key closer must be open before opening the circuit. When this option is enabled the circuit isn't considered open until the key closer has remained open continuously for almost a second (rather than immediately upon opening it). This option is primarily intended for use with a longer physical wire loop, where noise on the line might otherwise trigger a circuit open state.
...
This is Level -3 (minus 3)(((Error -3, Level)))

*Warn* – This is information about something that might cause, or be causing, the application to not operate as intended/desired.
This is Level -2 (minus 2)(((Warn -2, Level)))
...
This is Level -1 (minus 1)(((Info -1, Level)))

*Debug* – Debugging information is actually provided at a number of levels and is intended to help in troubleshooting problems in the operation of the application.
This is Level 1+ (1 and greater)(((Debug 1+, Level)))
...
* *1* (and greater) – Log Error, Warn, and Info level information, as well as Debug information designated as Level-X, where X is equal to or less than the value specified. The higher the number, the more debugging information that will be logged.(((higher number, Level, Debug)))

[%nonfacing]
=== Cancel, Apply, Save
The buttons at the bottom all close the Preferences dialog. They allow you to:

* *Cancel* – Do not make any changes to the active configuration (even if changes have been made within the dialog).
...
[%nonfacing]
== Creating a ((New Configuration)) or ((Updating an Existing Configuration))(((Configuration, New)))(((Configuration, Update existing)))
Using the Preferences dialog and the File menu, new named configurations can be created or an existing configuration can be updated.
...
The *((Configure))* (((Configure, Command line utility))) command line utility can also read from one configuration, make changes, and save to a different configuration. Use the `--help` command line option of the Configure utility to see information about its operation.

[%nonfacing]
== Starting ((MKOB)) with a ((Specific Configuration)) (and multiple instances)(((multiple instances, MKOB)))
...
(((pykob)))
This is done with a single installation of ((MKOB)) (pykob). There is no need to install ((MKOB)) (pykob) multiple times.(((no need, install multiple copies)))
...
.	Select the serial port used to connect the interface.
.	Select ‘Local loop’ as the equipment type.
.	Check ‘Use local sounder’.
.	Uncheck ‘Use system sound’.

Physical Key & Sounder connected through the interface, but not connected in series:

.	Select the serial port used to connect the interface.
.	Select ‘Separate key and sounder’ as the equipment type.
.	Check ‘Use local sounder’.
.	Uncheck ‘Use system sound’.
...
.	Select the serial port used to connect the interface.
.	Select ‘Separate key and sounder’ as the equipment type.
.	Uncheck ‘Use local sounder’.
.	Check ‘Use system sound’ to provide the synthesized sounder.
.	(if using ((International Code)), optionally select ‘Tone’ rather than ‘Sounder’)(((tone, International Code)))
...
.	Select ‘None’ for ‘Key and sounder interface’.
.	Uncheck ‘Use local sounder’.
.	Check ‘Use system sound’ to provide the synthesized sounder.
.	(if using ((International Code)), optionally select ‘Tone’ rather than ‘Sounder’)(((tone, International Code)))

Physical KOB connected using a ‘loop’ interface – for ‘private’ practice:

TIP: If using the Les, Chip, or SilkyDESIGN Loop interfaces footnote:loopinterfaces[The Les (circuit), Chip interface, and SilkyDESIGN interface have been use to demonstrate this functionality. It is possible that other KOB/Loop interfaces will also work in this manner. If you have a different interface, and it works in this way, let us know and it will be noted.]
and a KOB, ((MKOB)) allows you to disable the sounder, use the system sound (allowing you to use headphones), and still be able to key using the physical key on the KOB! This allows you to practice your physical keying without disturbing others in your house, even though your equipment is a Loop and KOB!!!

.	Select the serial port used to connect the interface.
.	Select ‘Local loop’ as the equipment type.
.	Uncheck ‘Use local sounder’.
.	Check ‘Use system sound’.
+
...
[%nonfacing]
=== Record
You can start recording a session using the File:Record menu entry. That will start recording the current session using an automatically generated file name (based on the current date and time). The name of the file will be presented in a dialog box and displayed in the ((MKOB)) status bar. The recording will be stored in the folder containing MKOB.(((recording, file location)))
...
[colophon]
= Colophon
This document reflects the features of ((MKOB)) ((Version 4.4.3))
